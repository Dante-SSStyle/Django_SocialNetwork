...
def image_ranking(request):
    ...
    image_ranking1 = r.zrange('image_ranking1', 0, -1, desc=True)[:10]
    image_ranking_ids = [int(id1) for id1 in image_ranking1]
    # Получаем отсортированный список самых популярных картинок.
    most_viewed = list(Image.objects.filter(id__in=image_ranking_ids))
    most_viewed.sort(key=lambda x: image_ranking_ids.index(x.id1))
